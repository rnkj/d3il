...
class RNN_Policy(nn.Module):
    ...
    def forward(self, inputs, rnn_states=None):
        ...
        # make prediction
        if self.training and self.model.network_type == "minrnn":
...
class RNN_Agent(BaseAgent):
    def __init__(
        ...
        self.eval_model_name = "eval_best_rnn.pth"
        self.last_model_name = "last_rnn.pth"
        ...
        for num_epoch in tqdm(range(self.epoch)):
            if not (num_epoch + 1) % self.eval_every_n_epochs:
                ...
                for data in self.test_dataloader:
                    state, action, mask = (
                        data  # [torch.squeeze(data[i]) for i in range(3)]
                    )
                ...
                log.info(
                    "Epoch {}: Mean test mse is {}".format(
                        num_epoch, avrg_test_mse
                    )
                )
                ...
                if avrg_test_mse < best_test_mse:
                    ...
                    log.info(
                        "New best test loss. Stored weights have been updated!"
                    )
            ...
            log.info(
                "Epoch {}: Average train loss is {}".format(
                    num_epoch, avrg_train_loss
                )
            )

        self.store_model_weights(
            self.working_dir, sv_name=self.last_model_name
        )
    ...
    def train_step(
        ...
        if goal is not None:
            ...
            out, _ = self.model(torch.cat([state, goal], dim=-1))
        else:
            out, _ = self.model(state)
    ...
    @torch.no_grad()
    def evaluate(
        self, state, action: torch.Tensor, goal: Optional[torch.Tensor] = None
    ):
        """
    ...
    def predict(
        ...
        if goal is not None:
            goal = self.scaler.scale_input(goal)
            out, self.rnn_states = self.model(
                torch.cat([state, goal], dim=-1), self.rnn_states
            )
        else:
            out, self.rnn_states = self.model(state, self.rnn_states)
