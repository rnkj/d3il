...
data_dir_files = [
    os.path.join(dataset_dir, "train_files.pkl"),
    os.path.join(dataset_dir, "eval_files.pkl"),
]
...
modes = np.load(
    os.path.join(dataset_dir, "all_modes.pkl"), allow_pickle=True
)
mode_prob = np.load(
    os.path.join(dataset_dir, "mode_prob.pkl"), allow_pickle=True
)
for key in modes.keys():
    hdf.attrs["mode_metadata"] = key
    hdf.attrs["all_modes"] = modes[key]
    hdf.attrs["mode_prob"] = mode_prob[key]
...
state_files = sorted(glob(os.path.join(dataset_dir, "vision_data/state/*.pkl")))
image_dirs = sorted(glob(os.path.join(dataset_dir, "vision_data/images/*")))
